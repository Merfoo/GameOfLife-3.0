...
function Map(tileArray)
{
    ...
    this.drawMap = function () {
        ...
        var rVal = document.getElementById("rSlide").value;
        var gVal = document.getElementById("gSlide").value;
        var bVal = document.getElementById("bSlide").value;
        for (var x = camx; x < camx + mapx; x++) {
            for (var y = camy; y < camy + mapy; y++) {
                ...
                _canvasContext.fillStyle = "rgba(" + rVal + ", " + gVal + ", " + bVal + ", " + this.tiles[x][y].population / 2 + ")";
...
function updateSand(rows) {
    ...
    for (var y = 0; y < map_size; y++) 
        for (var x = 0; x < map_size; x++) 
            if (rows[x][y] === landID || rows[x][y] === forestID || rows[x][y] === mountainID) 
                if (checkSurround(rows, x, y, waterID, 2)) 
                    rows[x][y] = sandID;
    ...
    //    for (var x = 0; x < map_size; x++)
    //        (rows[x][y] === landID || rows[x][y] === forestID || rows[x][y] === mountainID)
